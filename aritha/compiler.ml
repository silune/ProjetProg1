...
let code_of_op op =
        ...
        | DIVI _ -> (pop_int rdi) ++ (pop_int rax) ++ movq (imm 0) (reg rdx) ++ cqto ++ idivq (reg rdi) ++ (push_int rax)
        | MODI _ -> (pop_int rdi) ++ (pop_int rax) ++ movq (imm 0) (reg rdx) ++ cqto ++ idivq (reg rdi) ++ (push_int rdx)
